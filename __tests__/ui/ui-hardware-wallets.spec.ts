...
test.describe('Page Structure Tests', () => {
  ...
  test('should display the header with correct title and CTA button', async ({ page }) => {
    await expect(hwPage.header).toBeVisible()
    await expect(hwPage.header.locator('h1.heading-3')).toHaveText(/Hardware Wallet/i)
...
test.describe('Hardware Wallet Sections', () => {
  test('should display the hardware wallet collection section with correct title and 4 product cards', async () => {
    ...
    await expect(hwPage.hardwareWalletCollection.locator('h2.heading-3')).toHaveText(/Ledger Hardware Wallets/i)
    await expect(hwPage.hardwareWalletCollection.locator('[data-testid="hardware-wallet-card"]')).toHaveCount(4)
  })
  ...
  test('should display the hardware wallet comparison section with correct title', async () => {
    ...
    await expect(hwPage.hardwareWalletComparison.locator('h2.heading-3')).toHaveText(/Ledger Hardware Wallet Comparison/i)
  })
  ...
  test('should display the hardware wallet explained section with correct text', async () => {
    ...
    await expect(hwPage.hardwareWalletExplained).toHaveText(/Hardware wallets explained/i)
  })
  ...
  test('should display the secure seed phrase section with correct text', async () => {
    ...
    await expect(hwPage.secureSeedPhrase).toHaveText(/Secure your seed phrase/i)
  })
  ...
  test("should display the 'How to use a hardware wallet' section with correct title and 3 features", async () => {
    ...
    await expect(hwPage.howToUseHardwareWallet.locator('h2.heading-3')).toHaveText(/How to use a hardware wallet\?/i)
    await expect(hwPage.howToUseHardwareWallet.locator('[data-cs-override-id^="feature-details-"]')).toHaveCount(3)
  })
